...
def compute_lateral_speed(lateral_position, time_step= 1 / 30):
    """
...
def compute_multi_scale_dtw_similarities(X_test_windows, current_time, window_length):
    ...
    medium_scale = 30  # Your current window_length (25)
    long_scale = 60  # For longer-term patterns
...
def lateral_speed_scaling(feature_windows, current_time, dtw_dissimilarity, lateral_speeds):
    """
    Modified coefficients for higher sensitivity to small lateral movements.
    """
    ...
    if len(recent_speeds) > 0:
        ...
        # Coefficient 1: Lower the threshold from 0.02 to 0.015 for AD4CHE
        lateral_speed_threshold = 0.015

        if max_lateral_speed < lateral_speed_threshold:
            # Coefficient 2: Decrease base trust to 0.05 (was 0.1)
            # Coefficient 3: Increase sigmoid steepness from -15 to -25
            # Coefficient 4: Shift the midpoint to 0.8
            scaling_factor = max(0.05, max_lateral_speed / lateral_speed_threshold)
            smooth_scaling = 0.05 + 0.95 * (1 / (1 + np.exp(-25 * (scaling_factor - 0.8))))
        else:
            smooth_scaling = 1.0
...
class StateManager:
    ...
    def __init__(self, lc_commitment_period=15):
    ...
    def update_state(self, predicted_class, similarity, is_low_speed=False):
        ...
        if (is_lc_prediction and self.current_state == 'LC') or \
            ...
            if is_low_speed and is_lc_prediction and self.current_state == 'LK':
                ...
                self.consecutive_opposite_frames += 0.1  # Reduced increment
            else:
                # Normal increment
        ...
        if self.current_state == 'LK':
            ...
            if is_low_speed:
                threshold = 15  # Increased threshold for low speed
            else:
...
def improved_assess_prediction_with_density(pred_probs, classification_output,
    ...
    if len(recent_speeds) > 0:
        ...
        if max_lateral_speed < 0.02 and state_info['state'] == 'LC':
    ...
    ratio_score = 1 / (1 + np.exp(-0.9 * (np.log(density_ratio) - 1)))

    # Calculate a weighted combination of absolute density scores and ratio score
    alpha = 0.2  # Weight for absolute scores
    beta = 0.8  # Weight for ratio score
